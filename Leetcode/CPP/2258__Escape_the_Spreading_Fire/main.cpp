...
public:
    ...
    inline static constexpr int       const always  = 1'000'000'000;
public:
...
public:
    ...
    bool condition(vector<vector<int>>& grid, int const &minutes) {
        ...
        // std::fprintf(stdout, "minutes is [%d]\n", minutes);
        // std::ranges::for_each(fired, [&](std::vector<bool> const &row){
        //     for(bool const &val: row) { std::fprintf(stdout, "%d  ", val); }
        //     std::putc('\n', stdout);
        //     });
        // std::puts("- - - - - - - - - - - - - - - - - - - -");
