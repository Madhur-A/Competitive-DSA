...
rx(S1, S2) ->
    ...
    lists:map(fun({First, _}) -> binary:list_to_bin(First) end,
              dict:to_list(dict:filter(fun(_, V) -> V =:= 1 end,
                                       lists:foldl(fun(E, D) -> dict:update(E, fun(V) -> V + 1 end, 1, D) end,
                                                   dict:new(), S)))).

uncommon_from_sentences(S1, S2) -> rx(S1, S2).
