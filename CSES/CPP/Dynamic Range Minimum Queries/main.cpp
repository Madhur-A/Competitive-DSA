



#include <bits/stdc++.h>

int _ = [](){ std::cin.tie(nullptr); std::ios_base::sync_with_stdio(false); return 0; }();

using lnt = long long;

struct seg_tree {
public:
    lnt n;
    std::vector<lnt> nums, t;
public:
    inline seg_tree(std::vector<lnt> const &nums): n(nums.size()), nums(nums), t(4 * n, INT_MAX) {
        build(0, 0, n - 1);
    }

    inline void build(int node, int start, int finish) {
        if(start == finish) {
            t[node] = nums[start];
        } else {
            int const mid = start + ((finish - start) >> 1);
            build(2*node + 1, start, mid);
            build(2*node + 2, mid + 1, finish);
            t[node] = std::min(t[2*node + 1], t[2*node + 2]);
        }
    }

    inline void update(int index, int value) {
        return update(0, 0, n - 1, index, value);
    }

    inline void update(int node, int start, int finish, int index, int value) {
        if(start == finish) { t[node] = value; return; }
        int const mid = start + ((finish - start) >> 1);
        if(index <= mid) {
            update(2*node + 1, start, mid, index, value);
        } else {
            update(2*node + 2, mid + 1, finish, index, value);
        }
        t[node] = std::min(t[2*node + 1], t[2*node + 2]);
    }

    inline lnt range_min(int left, int right) {
        return range_min(0, 0, n - 1, left, right);
    }

    inline lnt range_min(int node, int start, int finish, int left, int right) {
        if(start > right or finish < left) { return INT_MAX; }
        if(left <= start and finish <= right) { return t[node]; }
        int const mid = start + ((finish - start) >> 1);
        return std::min(
                        range_min(2*node + 1, start, mid, left, right),
                        range_min(2*node + 2, mid + 1, finish, left, right)
                        );
    }
};


int main() {
    lnt n = 0, q = 0;
    std::cin >> n >> q;
    std::vector<lnt> nums(n);
    int num = 0, i = 0;
    while(n--) {
        std::cin >> num;
        nums[i++] = num;
    }
    lnt type = 0, to = 0, from = 0;
    seg_tree st(nums);
    while(q--) {
        std::cin >> type >> from >> to;
        if(type == 1) {
            st.update(from - 1, to);
        } else {
            std::cout << st.range_min(from - 1, to - 1) << '\n';
        }
    }
}
