...
public:
    lnt n;
    std::vector<lnt> nums, t, l;
public:
    inline seg_tree(std::vector<lnt> const &nums): n(nums.size()), nums(nums), t(4 * n), l(4 * n) {
        build(0, 0, n - 1);
    }
    ...
    inline lnt range_sum(int index) {
        return range_sum(0, 0, n - 1, index);
    }

    inline lnt range_sum(int node, int start, int finish, int index) {
        propagate(node, start, finish);
